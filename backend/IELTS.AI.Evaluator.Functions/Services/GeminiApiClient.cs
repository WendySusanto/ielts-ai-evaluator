...
namespace IELTS.AI.Evaluator.Functions.Services
{
    ...
    {
        Task<string> EvaluateEssayAsync(string userAnswer, string question, string? taskType, string apiKey);
    }
    ...
    public class GeminiApiClient : IGeminiApiClient
    {
        ...
        public async Task<string> EvaluateEssayAsync(string userAnswer, string question, string? taskType, string apiKey)
        {
            var promptTemplate =
                ...
                "- For the `issues` part, provide specific examples: what part of the text has a problem, and why. Please mention all the issues in array\n" +
            ...
            var prompt = string.Format(promptTemplate, taskType, question, userAnswer);

            var endpoint = _configuration["GeminiApiEndpoint"] ?? "https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent";
